...
LANGUAGE_INFO = {
    "fortran": {"file_pattern": "*.f"},
    "python": {"file_pattern": "*.py"},
    "c": {"file_pattern": "*.[ch]"},
}
...
def main(input_path, output_path, language, save_pdf, save_tex):
    ...
    if len(code_blocks) == 0:
        raise ValueError(
            f"The input folder has no files matching the `{glob_pattern}` pattern."
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Program to format code into PDFs using LaTeX"
    )
    ...
    parser.add_argument(
        "--save_tex",
        help="Whether to save the generated tex sourcecode",
        type=str2bool,
        default="False",
    )
    parser.add_argument(
        "--save_pdf",
        help="Whether to save compile a PDF",
        type=str2bool,
        default="True",
    )
