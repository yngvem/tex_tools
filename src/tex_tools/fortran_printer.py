...
def load_files(folder):
    return dict(load_file(fortran_file) for fortran_file in Path(folder).glob('*.f'))
...
def main(input_data, out_file):
    ...
    if len(code_blocks) == 0:
        raise ValueError('The input folder has no `.f` files.')

    template_path = Path(__file__).parent 
    template_name = 'fortran_template.tex'
